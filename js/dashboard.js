...
function renderDashboard(userData, userServices) {
    ...
    `).join('') : '<p class="text-muted">You have not listed any services yet.</p>';
            ...
            <div class="space-y-8">
                <div class="bg-white p-6 rounded-lg shadow-md slide-up">
                    ...
                    <form id="add-service-form">
                        ...
                        <div class="form-group"><label for="service-image-file" style="font-weight: 600; display: block; margin-bottom: 0.5rem;">Service Image</label><input type="file" id="service-image-file" accept="image/*" required></div>
                ...
                <div class="bg-white p-6 rounded-lg shadow-md slide-up" style="animation-delay: 0.2s;">
                    <h2 class="font-bold text-xl mb-4">My Profile</h2>
                     <form id="update-profile-form">
                        ...
                        <div class="form-group"><label for="profile-photo-file" style="font-weight: 600; display: block; margin-bottom: 0.5rem;">Update Profile Photo</label><input type="file" id="profile-photo-file" accept="image/*"></div>
...
async function loadDashboard() {
    ...
    } catch (error) { console.error("Error loading dashboard:", error); dashboardContainer.innerHTML = '<p class="text-center text-error-color">Could not load dashboard. Please try again.</p>'; }
}
...
function attachEventListeners(currentUserName) {
    ...
    document.getElementById('add-service-form').addEventListener('submit', async (e) => {
        ...
        if (!imageFile) { alert('Please select an image file.'); submitButton.disabled = false; submitButton.textContent = 'Add Service'; return; }
        try {
            ...
            e.target.reset();
            loadDashboard();
        } catch(error) { console.error("Error adding service:", error); alert("Could not add service."); } 
        finally { submitButton.disabled = false; submitButton.textContent = 'Add Service'; }
    });
    ...
    document.getElementById('update-profile-form').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            loadDashboard();
        } catch (error) { alert("Could not update profile."); }
        finally { submitButton.disabled = false; submitButton.textContent = 'Update Profile'; }
    });
    
    document.querySelectorAll('.delete-service-btn').forEach(button => {
        button.addEventListener('click', async (e) => {
            if (confirm("Are you sure you want to delete this service?")) {
                try { await deleteDoc(doc(db, "services", e.target.dataset.id)); loadDashboard(); } 
                catch (error) { alert("Could not delete service."); }
            }
        });
    });
}